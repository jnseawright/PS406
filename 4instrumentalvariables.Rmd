...
```{r wright_sim_2, echo = TRUE, fig.height = 4}
...
```{r wright_sim_3, echo = TRUE, fig.height = 4}
...
-   The Anderson–Rubin 95% confidence interval for `gdp_g` is `[`r round(ar_ci[2,1], 3)`, `r round(ar_ci[2,2], 3)`]`. This interval is valid even if instruments are weak.

-   Compare with the conventional IV confidence interval: `r round(confint(migueliv)[2,1], 3)` to `r round(confint(migueliv)[2,2], 3)`. The AR interval is slightly wider when instruments are weak.
...
-   The manual IV estimate is `r round(wald_iv, 3)`.  
    This suggests that for compliers, feeling anger increases the probability of outsider voting by about 157 percentage points. This is obviously too high, and we probably have either some measurement error in the compliance rate or a huge standard error.
...
-   The ATE estimate for the effect of anger on outsider voting is `r round(exp.reweight$coefficients, 3)`.
