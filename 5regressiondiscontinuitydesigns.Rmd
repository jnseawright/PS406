...
- Density testing: The McCrary/rddensity test for manipulation assumes the running variable is continuously distributed. With mass points, the density is lumpy by construction, which can produce false positives. rddensity handles this automatically by adjusting the estimator, but you should be aware the standard version of the test is unreliable here.
- Bin construction: rdplot() adjusts its binning algorithm when mass points are detected, grouping at the level of unique values rather than constructing equal-width bins. The plot you see already reflects this adjustment.
