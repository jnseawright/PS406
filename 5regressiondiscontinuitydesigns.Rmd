...
for (i in seq_along(placebo_cutoffs)) {
  rd_tmp <- rdrobust(lmb_data$score, lmb_data$lagdemvoteshare,
                     c = placebo_cutoffs[i])
  placebo_results[i, "est"]     <- rd_tmp$Estimate[1, "tau.bc"]
...
true_est    <- rd_ll$Estimate[1, "tau.bc"]
...
for (i in seq_along(mult)) {
  rd_tmp <- rdrobust(lmb_data$score, lmb_data$lagdemvoteshare, c = 0.5, 
                     h = bw_opt * mult[i])
  results[i, "est"] <- rd_tmp$Estimate[1, "tau.bc"]   
...
results_df <- data.frame(
  ...
  Estimate = round(c(rd_opt$Estimate[2], rd_half$Estimate[2], rd_double$Estimate[2], rd_quad$Estimate[2]), 3),
