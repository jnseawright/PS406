...
**Interpretation**: The p-value > 0.05 suggests that we cannot reject the null hypothesis of parallel pre-treatment trends. This provides some support for the parallel trends assumption. However, the relatively low p-value suggests that there is some evidence of divergent pre-treatment trends, so we should not rest easy.
...
| Question | Answer |
|----------|--------|
| Estimated effect | `r round(100*coef(clfe)["TreatedTRUE"], 2)` percentage point decrease |
...
The true average treatment effect on the treated (ATT) is about `(0.3*2 + 0.3*1)/0.6 = 1.5`, but TWFE gives `r round(coef(twfe), 3)` which is slightly biased downward.
