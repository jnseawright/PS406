...
for (i in 1:100){
  perutemp <- peruemotions[rbern(length(missing.prob),missing.prob)==1,]
...
2. Choose one variable ($X_{i}$) to work on. Use the initial guesses, as well as the observed values, for all the other variables to run a regression with $X_{i}$ as the outcome and $X_{-i}$ as the predictors. Draw predictions from that regression for the missing values on $X_{i}$.
