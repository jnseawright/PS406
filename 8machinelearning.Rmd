...
### Why AIPW? The Case for Doubly Robust Estimation

To combine the leaf-level estimates from a causal forest into a single,
valid estimate of the ATE or CATE, we need an estimator with good
properties even when our nuisance functions are imperfectly estimated.

The **Augmented Inverse Probability Weighted (AIPW)** estimator has a
crucial property: it is **doubly robust**. 
-  It remains consistent if *either* the outcome model $\hat{m}(X)$ *or* the propensity score
$\hat{e}(X)$ is estimated consistently.

---

- The **IPW term** reweights observations to balance treatment and control.
- The **augmentation term** corrects for bias in the IPW estimate using the outcome model.

Together, they provide valid inference even when machine learning
estimators converge slowly.
...
### Reading the AIPW Formula

The formula shown above combines two components for each unit $i$:

$$\hat{\tau}^{AIPW} = \frac{1}{n}\sum_i \left[ \hat{\tau}(X_i) + \frac{W_i - \hat{e}(X_i)}{\hat{e}(X_i)(1-\hat{e}(X_i))} \left(Y_i - \hat{m}(X_i)\right) \right]$$

- The first term $\hat{\tau}(X_i)$ is the causal forest's direct estimate
  of the CATE at $X_i$.
- The second term is a bias correction: it is large when the residual
  $(Y_i - \hat{m}(X_i))$ is large *and* when the unit's treatment
  probability was far from its propensity score.

---

`get_scores()` in the `grf` package returns AIPW scores. They are the per-unit contributions to this doubly robust estimate.

---

Using the AIPW to combine estimates that derive from causal forests gives us the much-desired doubly robust estimator.
...
- Text methods rely on computational infrastructure and often benefit from integration with the methods discussed above that are flexible with high-dimensional data.
